...
const EditCoupon = async (req, res) => {
    try {
      const { code, discountValue, expiryDate, minPurchaseAmount, maxPurchaseAmount,usageLimit } = req.body;
      const  { id } = req.params;
...
const FilterSalesReport = async (req, res) => {
  ...
  const LoadDashboardWeekly = async (req, res) => {
    try {
        ...
        const validWeeklyOrders =  weeklyOrders.filter(order => order.orderStatus !== 'Canceled' && order.orderStatus !== 'order returned')    
        
        const revenue = validWeeklyOrders.reduce((total, order) => total + order.amount, 0);
...
const LoadDashboardYearly = async (req, res) => {
  try {
      ...
      const validyearlyOrders =  yearlyOrders.filter(order => order.orderStatus !== 'Canceled' && order.orderStatus !== 'order returned')    

      const revenue = validyearlyOrders.reduce((total, order) => total + order.amount, 0);
...
const LoadDashboardDaily = async (req, res) => {
  try {
      ...
      const validDailyOrders =  dailyOrders.filter(order => order.orderStatus !== 'Canceled' && order.orderStatus !== 'order returned')    

      const revenue = validDailyOrders.reduce((total, order) => total + order.amount, 0);
